...
//cp HTML
gulp.task("cphtml:dist", function() {
  return gulp
    .src(paths.srcHTML)
    .pipe(
      realFavicon.injectFaviconMarkups(
...
// process SCSS
gulp.task("processSCSS:dist", function() {
  return gulp
    .src(paths.srcSCSS)
    .pipe(
      sass
        .sync({ errLogToConsole: true, outputStyle: "compressed" })
        .on("error", sass.logError)
    )
    .pipe(postcss(postcssProcessors))
    .pipe(gulp.dest(paths.dist));
...
gulp.task("serve", ["build", "watchSrc"], function() {
...
gulp.task("build", ["cphtml", "cpimg", "processSCSS", "cpjs", "cpfav"]);
